...
class patientsoldd extends StatefulWidget {
  const patientsoldd({super.key});

  @override
  State<patientsoldd> createState() => _patientsolddState();
}

class _patientsolddState extends State<patientsoldd> {
...
//get list data
  ...
  listpatients(TextEditingController search) async {
    ...
    var response = _search.text != ""
        ? await _crud
            .getRequest('${ApisearchPatientsdate}?phone=${search.text}')
        : await _crud.getRequest(Apipatientsdate);
    setState(() {
  ...
  //next to ready
  nexttoreadypatients(String username, String phone, String birthday,
    ...
    var response = await _crud.getRequest(
        '${ApiinsertPatientsdate}?username=$username&phone=$phone&birthday=$birthday&datenow=${datenow}&timenow=${timenow}&gender=$gender&price=$price&useradd=$useradd');
    listpatient.clear();
    listpatients(_search);
    isloding = false;
  }
